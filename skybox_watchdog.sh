...
if [[ "$PROXYVAR" == true ]]; then
    ...
    if [ "$?" -eq "0" ]; then
       ...
       echo `date +"%Y/%m/%d %H:%M:%S - The server did not respond correctly to the REST healthcheck. Curl error code $RESPONSE for port $2 was received."` | tee -a $SKYBOXLOG $WATCHDOGLOG
       RESULT=1
    fi
fi
return $RESULT
